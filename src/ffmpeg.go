...
func FfmpegDownload(url string, username string, output string, errorMsg string) {
	...
	watermark := fmt.Sprintf("drawtext='text=%v:expansion=normal:fontfile=lib/roboto/Roboto-Regular.ttf:y=h-line_h-10:x=w-mod(max(t-4.5\\,0)*(w+tw)/255.5\\,(w+tw+7500)):fontcolor=white:fontsize=40:shadowx=2:shadowy=2'", disclaimer)
